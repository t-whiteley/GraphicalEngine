...
void Screen::set_point(double x, double y, double z, sf::RenderWindow& window) {
...
void Screen::set_line(float x1, float y1, float z1, float x2, float y2, float z2, sf::RenderWindow& window) {
    double grid_x1 = x1;
    double grid_y1 = y1;
    double grid_x2 = x2;
    double grid_y2 = y2;
    ...
    if (std::abs(grid_x1 - grid_x2) < std::numeric_limits<double>::epsilon()) {
        ...
        while (y <= std::max(grid_y1, grid_y2)) {
            set_point(grid_x1, y, lerp(z1, z2, (y - grid_y1) / (grid_y2 - grid_y1)), window);
            y += 0.001;
        }
        return;
    }
...
struct Mesh Screen::mesh_to_view_volume(struct Mesh mesh) {
    ...
    for (int i = 0; i < mesh.tris.size(); i++) {
        for (int j = 0; j < 3; j++) {
            ...
            Matrix new_v = ToScreenSpace.multiply(old_v); // THIS IS WHY; IS THE DERIVATION CORRECT?? Z AND W ALWAYS THE SAME, MULTIPLICATION FUNCTION IS CORRECT
            
            // printf("conversion matrix:");
            // ToScreenSpace.print();
            
            // printf("multiplied coords:");
            // new_v.print();
...
void Screen::draw_mesh(struct Mesh mesh, sf::RenderWindow& window) {
    ...
    for (int i = 0; i < mesh.tris.size(); i++) {
        for (int j = 0; j < 3; j++) {
            ...
            set_line(x1, y1, z1, x2, y2, z2, window);
        }
    }
}
