...
Camera::Camera() : 
    ...
    ToCamSpace(4, 4) {   
        ...
        cam_position.vals[0][0] = 0;
        cam_position.vals[1][0] = 0;
        cam_position.vals[2][0] = 0;
        cam_pitch = 0;
        cam_yaw = 0;
...
Matrix Camera::Build_ToCamSpace() {
    ...
    // std::cout << "cam_fx: " << cos(cam_pitch) * sin(cam_yaw) << ". \n";
    // std::cout << "cam_fy: " << sin(cam_pitch) << ". \n";
    // std::cout << "cam_fz: " << cos(cam_pitch) * cos(cam_yaw) << ". \n";
    // std::cout << "tot_pitch: " << cam_pitch << ", tot_yaw: " << cam_yaw << ". \n";
    // cam_f.print();
...
struct Mesh Camera::mesh_to_cam_space(struct Mesh mesh) {
    ...
    for (int i = 0; i < mesh.tris.size(); i++) {
        for (int j = 0; j < 3; j++) {
            ...
            // HG_node.print();
            // mesh.tris[i].nodes[j] = ToCamSpace.multiply(HG_node);
            mesh.tris[i].nodes[j].vals[0][0] = HG_node.vals[0][0];
            mesh.tris[i].nodes[j].vals[1][0] = HG_node.vals[1][0];
            mesh.tris[i].nodes[j].vals[2][0] = HG_node.vals[2][0];
        }
    }
    return mesh;
}